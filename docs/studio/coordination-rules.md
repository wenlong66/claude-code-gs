...
Skills and agents are assigned to model tiers based on spawn_agent complexity:
...
via a shared spawn_agent list. Each session has its own context window and token budget.
...
- The spawn_agent fits in a single session's context (use subagents instead)
