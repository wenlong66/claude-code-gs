...
`agents/`. Use the agent best suited to the spawn_agent at hand. When a spawn_agent
spans multiple domains, the coordinating agent (usually `producer` or the
domain lead) should delegate to specialists.

Codex compatibility note: these files are role context cards, not guaranteed
runtime registrations for named agents. When a workflow says to spawn or consult
`agent-name`, first read `agents/agent-name.toml`; then either pass that role
context to Codex `spawn_agent` if available, or apply the role's review criteria
