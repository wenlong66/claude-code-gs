...
**FAIL** if the skill instructs file creation or edits but no ask-before-write language is found.
...
### Check 7 — Argument Documentation Plausibility
If the skill body mentions multiple modes or arguments (e.g., "Mode A | Mode B"),
the first phase should document how to parse them and show usage examples.

**WARN** if documented modes are missing, contradictory, or do not match the
first phase's "Parse Arguments" section.
