...
**CRITICAL: spawn_agent in this skill spawns a SUBAGENT — a separate independent Codex session
with its own context window. It is NOT spawn_agent tracking. Do NOT simulate specialist
...
When the verdict is APPROVED, use a single `request_user_input` with `multiSelect: true` to batch the two tracking updates:
