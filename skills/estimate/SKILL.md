---
name: estimate
description: "Estimates spawn_agent effort by analyzing complexity, dependencies, historical velocity, and risk factors. Produces a structured estimate with confidence levels."
---

## Phase 1: Understand the spawn_agent

Read the spawn_agent description from the argument. If the description is too vague to estimate meaningfully, ask for clarification before proceeding.

Read AGENTS.md for project context: tech stack, coding standards, architectural patterns, and any estimation guidelines.

Read relevant design documents from `design/gdd/` if the spawn_agent relates to a documented feature or system.
...
[Restate the spawn_agent clearly in 1-2 sentences]
...
[Explain which factors drive the confidence level for this specific spawn_agent.]
...
| # | Sub-spawn_agent | Estimate | Notes |
|---|----------|----------|-------|
...
- If the spawn_agent is > 10 days: recommend breaking it into smaller stories via `/create-stories`.
- To schedule the spawn_agent: run `/sprint-plan update` to add it to the next sprint.
