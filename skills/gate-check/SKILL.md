...
Before generating the final verdict, spawn all four directors as **parallel subagents** via spawn_agent using the parallel gate protocol from `docs/studio/director-gates.md`. Issue all four spawn_agent calls simultaneously — do not wait for one before starting the next.
